...
public interface ProductRepository extends JpaRepository<Product, String>{
	// 커스텀 쿼리
	// JPArepo : 자바 문법으로 db에 요청할 수 있도록 준비되어있는 메서드들을 활용해서 처리
	//  -> 하지만 현실적으로 쿼리 자체가 간단하게 나오는 경우보다 복잡하게 나오는 경우도 상당수
...
//	List<Product> findProduct1(@Param("minPrice") BigDecimal minPrice);
	...
	//    JPQL보다 조회라던지 표현 범위는 훨씬 넓기 때문에
	//    특정 DB의 고유 기능을 사용할때는 Native Query가 좋다
	
	// 직접 쿼리를 작성하는 방법
	// 1. JPQL : 문자열 기반 마법주문서
	// 장점 : 직관적이고 간단한 쿼리를 빠르게 작성할때 편함
	// 단점 : 쿼일에 오타가 있거나 문법 오류가 있어도 에러메서지를 확인하는게 어려움
	//  -> 컴파일 시점에 발견이 절대 안됨
	//  -> 자바의 장점을 스스로 포기하는 행동
	// 2. QueryDSL : 자바 코드로 짓는 쿼리의 시
	//  -> 쿼리를 문자열이 아닌 자바코드로 작성하는 방식
	//  -> 서버 실행할때 문제있으면 발견됨
	//  -> 자동완성 기능도 사용 가능. 안전성 측면에서 큰 장점을 가져감
	//  -> 기존 자바 문법과의 조합도 가능
	//  단점 : 초기 세팅이 귀찮을 수 있음
